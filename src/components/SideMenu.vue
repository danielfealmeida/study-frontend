    ...
    export default {
        ...
        methods: {
            ...
            secondsToHms: function(time) {
                ...
                return h > 0 ? this.pad(h) + ":" : "" + this.pad(m) + ":" + this.pad(s)
            }
        }
    }
</script>
...
<template>
    <div class="side fixed right-0 w-60 h-full border-b-2 border-l-2 border-stone-200 dark:bg-stone-800 dark:text-white dark:border-b-1 dark:border-l-1 dark:border-dark-line align-middle text-center pt-5 text-sm">
        ...
        <div v-if="opened=='pomodoro'" class="mt-5">
            ...
            <button class="w-32 h-32 rounded-full border-2 border-stone-200 dark:bg-stone-800 dark:text-white dark:border-b-1 dark:border-l-1 dark:border-dark-line" :style="timer==0 && styleCount%2==0 && active ? {background: 'rgb(239 68 68)'} : '' " @click="active = !active; timer = timeSelected">{{ active ? secondsToHms(timer) : "Iniciar" }}</button>
